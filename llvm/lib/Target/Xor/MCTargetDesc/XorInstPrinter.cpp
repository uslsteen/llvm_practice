...
#include "XorGenAsmWriter.inc"

void XorInstPrinter::printRegName(raw_ostream &O, unsigned RegNo) const {
  O << getRegisterName(RegNo);
}

void XorInstPrinter::printInst(const MCInst *MI, uint64_t Address,
...
void XorInstPrinter::printOperand(const MCInst *MI, int OpNo, raw_ostream &O) {
...
void XorInstPrinter::printBranchOperand(const MCInst *MI, uint64_t Address,
...
const char *XorInstPrinter::getRegisterName(unsigned RegNo) {
  return getRegisterName(RegNo,
                         ArchRegNames ? Xor::NoRegAltName : Xor::ABIRegAltName);
}
